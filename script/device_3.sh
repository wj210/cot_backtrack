...
# srun -p $p -w $w -c $c --verbose --job-name=self_learning --gpus=$num_gpu bash script/run_3.sh
srun -p $p -w $w -c $c --verbose --job-name=self_learning --gpus=$num_gpu --pty ./script/run_3.sh
